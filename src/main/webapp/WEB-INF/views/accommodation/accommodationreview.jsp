...
<script>
		...
		$(data)
				.each(
						function() {
							...
							str += '<span class="created-at">' + created_at
									+ '</span>';
							str += '</div>';
							str += '<div class="review-content">' + rText
									+ '</div>';
							if (images) {
								str += '<img src="${path}/displayFile?fileName='
										+ images
										+ '" alt="숙소 이미지" class="review-image" />';
							}
							...
							str += '<button onclick="modDiv(this,' + rno
									+ ', \'' + rAuth + '\', \'' + rText
									+ '\');">수정</button>';
							str += '</div>';
							str += "</li>";
	...
	// 수정창 열기
	function modDiv(el, review_id, member_num, content) {
		$("#modRno").html(review_id);
		$("#modText").val(content);
