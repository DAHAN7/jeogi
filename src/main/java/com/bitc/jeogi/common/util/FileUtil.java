...
public class FileUtil {
	...
	/**
	 * 업로드 경로 와 파일이름을 매개변수로 전달 받아 지정된 위치에 파일 정보를 byte[] 이진데이터로 반환
	 */
	public static byte[] getBytes(String realPath, String fileName) throws Exception {
		...
		/*
		 * byte[] bytes = new byte[is.available()]; for(int i=0; i < bytes.length; i++)
	...
	public static HttpHeaders getHeaders(String fileName) throws Exception {
		...
		/*
		 * 8비트 / 1byte 단위의 이진 데이터 임을 의미 옥텟은 컴퓨팅에서 8개의 비트가 하나로 모인것을 의미 브라우저는 이진데이터는 해석할 수
		 * 없으므로 다운로드를 받아 사용자가 처리하도록 데이터를 처리한다.
		 */
		...
		/*
		 * // URL이 해석할수 있는 인코딩 형식으로 변경 origin = new String(origin.getBytes("UTF-8"),
		 * "ISO-8859-1"); // 컨텐츠 배치 옵션 // attachment : 첨부파일 // fileName 옵션 - 전송되는 첨부
		 * 데이터의 이름 명시 headers.add("Content-Disposition",
		...
		ContentDisposition cd = ContentDisposition.attachment()
				...
				// 언어 형식 지정
				.filename(origin, Charset.forName("UTF-8")).build();
